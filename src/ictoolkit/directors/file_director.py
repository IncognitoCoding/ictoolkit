...
def write_file(file_path: str, write_value: str) -> None:
    ...
    try:
        type_check(value=file_path, required_type=str)
        type_check(value=write_value, required_type=str)
    except FTypeError:
        raise
...
def search_file(
    ...
    Raises:
        ...
        \t\\- A general failure occurred while searching the file.
    ...
    try:
        type_check(value=file_path, required_type=(str, list))
        type_check(value=searching_value, required_type=(str, list))
    except FTypeError:
        raise
    ...
    try:
        ...
        exc_args = {
            "main_message": "A general failure occurred while searching the file.",
            "original_exception": exc,
        }
...
def convert_relative_to_full_path(relative_path: str) -> str:
    ...
    try:
        type_check(value=relative_path, required_type=str)
    except FTypeError:
        raise
...
def user_file_selection(prompt: str, criteria: str, root_dir: Optional[str] = None) -> str:
    ...
    try:
        type_check(value=prompt, required_type=str)
        type_check(value=criteria, required_type=str)
        if root_dir:
            type_check(value=root_dir, required_type=str)
    except FTypeError:
        raise
